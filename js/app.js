...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    <img class="product-image" src=${image}></img>
      ...
      <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="buy-now btn" style="background: gray;color: white;">add to cart</button>
      <button class="btn" style="background:olivedrab;color: white;" id="details-btn">Details</button>
      <p class =""><span style="color: orange;" ><i class="fas fa-star"></i><i class="fas fa-star-half-alt"></i> <i class="far fa-star"></i></span> ${product.rating.rate}</p>
      <p><i  style="color: cornflowerblue;" class="fas fa-user-check"></i> ${product.rating.count}</p>
      </div>
...
const updateTotal = () => {
  ...
  const convertGrandTotal =  parseFloat(grandTotal.toFixed(2));
  document.getElementById("total").innerText = convertGrandTotal;
};
loadProducts();
